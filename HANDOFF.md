...
매핑, 관계별 Evidence, TAXII 비동기 입력 경계. 세부사항은 [docs/stix.md](docs/stix.md)를
참고합니다.

검증: Python 3.12 Docker 환경에서 ruff, mypy, pytest 43개, 커버리지 100% 통과.

다음 세션은 `develop`에서 `feat/ioc-normalization`을 만들고 canonical IOC identity, 중복
제거, 민감 값 마스킹, STIX Indicator 병합을 구현합니다. 커밋은
`feat(ioc): add indicator normalization pipeline`, PR 대상은 `develop`입니다.
